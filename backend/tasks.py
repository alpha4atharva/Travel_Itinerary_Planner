...
def create_logistics_task(logistics_manager) -> Task:
    ...
    return Task(
        description=(
            ...
            "1. **Travel (Flights/Trains/Buses) — ROUND-TRIP:** Find AT LEAST 3 different travel options.\n"
            "   For EACH option, provide:\n"
            "   - **Outbound:** {origin} → {destination} — Airline/Train name, class, and price PER PERSON.\n"
            "   - **Return:** {destination} → {origin} — Airline/Train name, class, and price PER PERSON.\n"
            "   - **Round-trip total per person.**\n"
            "   Include a mix of budget and premium options.\n"
            "   Do NOT just list one-way prices. You MUST include BOTH outbound AND return.\n\n"
            "2. **Accommodation:** Find AT LEAST 3 specific hotels or guesthouses in {destination} with different price tiers. "
            ...
            "   - 💰 **Budget Package:** Cheapest ROUND-TRIP flight × {num_persons} + budget hotel + daily expenses × {num_persons} = TOTAL\n"
            "   - ⭐ **Mid-Range Package:** Mid-price ROUND-TRIP flight × {num_persons} + mid-range hotel + daily expenses × {num_persons} = TOTAL\n"
            "   - 👑 **Premium Package:** Best ROUND-TRIP flight × {num_persons} + premium hotel + daily expenses × {num_persons} = TOTAL\n\n"
        ...
        expected_output=(
            ...
            "**✈️ Round-Trip Flight / Train Options (comparison table — price per person):**\n"
            ...
            "| 1 | {destination} → {origin} | [Name] | Economy | {currency_symbol}XXX | local XXX |\n"
            "| 1 | **Round-trip total** | | | **{currency_symbol}XXX** | **local XXX** |\n"
            "| 2 | ... | ... | ... | ... | ... |\n"
            "| 3 | ... | ... | ... | ... | ... |\n\n"
            ...
            "**📊 Package Comparison (total for {num_persons} person(s), including round-trip flights):**\n"
            "| Package | Round-trip Flights | Hotel | Daily Exp. | TOTAL | Budget Status |\n"
...
def create_compilation_task(compiler, context_tasks: list) -> Task:
    ...
    return Task(
        ...
        expected_output=(
            ...
            "- Flight/Train Options comparison table (3+ options, dual currency)\n"
            "- Hotel Options comparison table (3+ options with names and prices, dual currency)\n"
